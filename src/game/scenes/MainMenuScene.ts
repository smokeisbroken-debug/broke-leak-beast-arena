...
export class MainMenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const nav = [
      ...
      { label: "BALANCE", sub: "DEBUG", x: 750, y: 166, color: 0x8cdcff, callback: () => this.scene.start(SCENE_KEYS.balanceDebug) },
      { label: "DUEL", sub: "1 VS 1 PREVIEW", x: 750, y: 212, color: 0xff7aeb, callback: () => this.scene.start(SCENE_KEYS.duel) },
      { label: "GUIDE", sub: "HOW TO PLAY", x: 750, y: 258, color: 0xfcfff7, callback: () => this.openGuideOverlay() },
      ...
      { label: "TOURNEY", sub: "LOCAL PREVIEW", x: 750, y: 396, color: 0xb66cff, callback: () => this.scene.start(SCENE_KEYS.tournament) },
    ];
